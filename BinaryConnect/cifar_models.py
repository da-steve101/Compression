...
class Cifar_Model:
    def __init__( self, inputs, target, filter_sizes ):
        ...
        self.layer_masks = []
        # use relu_max here?
        # self.activation_fn = lambda x: T.clip( lasagne.nonlinearities.rectify(x), 0 , 1 )
        self.activation_fn = lasagne.nonlinearities.rectify
        
    @staticmethod
    def getNonLinearLayer( cnn ):
        return lasagne.layers.NonlinearityLayer(
            cnn,
            nonlinearity = self.activation_fn )
    ...
    def build_model( self ):
        ...
        cnn = self.getNonLinearLayer( cnn )
        ...
        # squared hinge loss
        loss = T.mean( T.sqr ( T.maximum( 0.0, 1.0 - self.target*train_output ) ) )
        ...
        if self.binary:
            ...
            optimizer = lasagne.updates.adam( loss_or_grads = loss, params = params, learning_rate = self.LR )
            updates = OrderedDict( updates.items() + optimizer.items() )
        else:
            params = lasagne.layers.get_all_params( cnn, trainable = True )
            updates = lasagne.updates.adam( loss_or_grads = loss, params = params, learning_rate = self.LR )

        test_output = lasagne.layers.get_output( cnn, deterministic = True )
        test_loss = T.mean( T.sqr( T.maximum( 0.0, 1.0 - self.target*test_output ) ) ) + HW
    ...
    def build_model_prune_and_L2( self ):
